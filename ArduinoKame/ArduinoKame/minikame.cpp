...
void MiniKame::zero(){
    for (u8 i=0; i<8; i++) setServo(i, 90);
}
...
void MiniKame::moveServos(int time, float target[8]) {
    if (time>10){
        for (u8 i = 0; i < 8; i++)	_increment[i] = (target[i] - _servo_position[i]) / (time / 10.0);
        _final_time =  millis() + time;

        while (millis() < _final_time){
            _partial_time = millis() + 10;
            for (u8 i = 0; i < 8; i++) setServo(i, _servo_position[i] + _increment[i]);
            while (millis() < _partial_time); //pause
        }
    }
    else{
        for (u8 i = 0; i < 8; i++) setServo(i, target[i]);
    }
    for (u8 i = 0; i < 8; i++) _servo_position[i] = target[i];
}
...
void MiniKame::execute(float steps, float period[8], int amplitude[8], int offset[8], int phase[8]){
    ...
    for (u8 i=0; i<8; i++) oscillator[i].setTime(global_time);
